...
acelerado con Numba, incluyendo reglas de gestión de sesión y filtro EMA.
"""
import numpy as np
...
def run_fast_backtest(
    ...
    max_retest_candles: int, risk_reward_ratio: float, sl_lookback: int,
    initial_capital: float, commission_per_side: float, leverage: float,
    ema_filter_mode: int, # 0: Desactivado, 1: Moderado, 2: Fuerte
    ema_periods: np.ndarray
):
    num_candles = len(close_prices)
    num_levels = len(levels_prices)
    ...
    position_size, entry_price, direction, current_sl, current_tp = 0, 0.0, 0, 0.0, 0.0
    ...
    for i in range(1, num_candles):
        ...
        if direction != 0:
            ...
            if not is_closed and current_hour >= 13: is_closed, exit_price, exit_reason = True, close_prices[i], 3
            ...
            if is_closed and trade_count < max_trades:
                ...
                cash += pnl_gross - commission_per_side
                equity += pnl_net
                closed_trades[trade_count, :] = [timestamps[break_candle_indices.max()], timestamps[i], direction, position_size, entry_price, exit_price, pnl_net, exit_reason]
                trade_count += 1
                direction, position_size = 0, 0
                trades_today += 1
                if pnl_net > 0: stop_trading_for_day = True
                elif trades_today == 1 and pnl_net <= -60.0: stop_trading_for_day = True
                elif trades_today >= 2: stop_trading_for_day = True
                fsm_states[:] = IDLE
                break_candle_indices[:] = -1

        is_trading_window = (current_hour == 9 and current_minute >= 30) or (current_hour == 10) or (current_hour == 11 and current_minute <= 30)
        ...
        if direction == 0 and not stop_trading_for_day and is_trading_window:
            ...
            for j in range(num_levels):
                level_p, level_dir = levels_prices[j], levels_directions[j]
                if fsm_states[j] == IDLE:
                    ...
                    if is_break: fsm_states[j], break_candle_indices[j] = BROKEN, i
                elif fsm_states[j] == BROKEN:
                    if i - break_candle_indices[j] > max_retest_candles: fsm_states[j] = INVALIDATED; continue
                    retest_detected = (level_dir == 1 and low_prices[i] <= level_p and close_prices[i] > level_p) or \
                                      (level_dir == -1 and high_prices[i] >= level_p and close_prices[i] < level_p)
                    if retest_detected:
                        passes_ema = False
                        if ema_filter_mode == 0: passes_ema = True
                            ...
                            if not (np.isnan(s) or np.isnan(m) or np.isnan(l)):
                                ...
                                elif level_dir == -1: # Venta
                                    strong, moderate = s < m and m < l, close_prices[i] < m and s < m
                                    if ema_filter_mode == 1: passes_ema = moderate
                                    elif ema_filter_mode == 2: passes_ema = strong
                        if not passes_ema: fsm_states[j] = INVALIDATED; continue

                        direction, entry_price = level_dir, close_prices[i]
                        sl_start_idx = max(0, break_candle_indices[j] - sl_lookback)
                        if direction == 1: current_sl = np.min(low_prices[sl_start_idx : break_candle_indices[j] + 1])
                        else: current_sl = np.max(high_prices[sl_start_idx : break_candle_indices[j] + 1])
                        risk_dist = abs(entry_price - current_sl)
                        if risk_dist > 1e-9: current_tp = entry_price + (risk_dist * risk_reward_ratio) if direction == 1 else entry_price - (risk_dist * risk_reward_ratio)
                        else: direction = 0; continue
                        position_size = np.floor((equity * leverage) / entry_price)
                        if position_size > 0: cash -= commission_per_side; fsm_states[:] = INVALIDATED; break
                        else: direction = 0
                
        unrealized_pnl = (close_prices[i] - entry_price) * position_size if direction == 1 else (entry_price - close_prices[i]) * position_size if direction == -1 else 0.0
